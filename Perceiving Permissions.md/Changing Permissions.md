...
First check for permissions of flag and then as for this challenge chmod is all powerful and we don't know if we are in grp of root or not so make /flag readable for all others hence 
use chmod o+r /flag then read the flag using cat

```bash
hacker@permissions~changing-permissions:~$ ls -l /flag
-r--r----- 1 root root 60 Oct  7 13:48 /flag
hacker@permissions~changing-permissions:~$ chmod o+r /flag
hacker@permissions~changing-permissions:~$ cat /flag
pwn.college{YvKP0dDuc5QYjSvk47jTwWqoqS_.QXzcjM1wiNxIzNzEzW}
```

### New Learnings
using chmod user_initial+access file_name we can change permission of access to file, but only if we can write to the file.
