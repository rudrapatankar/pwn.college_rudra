...
### Solve
**Flag:** pwn.college{gT5TKmMkBAvSSgRvdHTAw-zdLQg.QX4UDN1wiNxIzNzEzW}
sudo only used to open files or used on files not on commands. Hence here we get flag using cat /flag and not /challenge/run as /flag is file and /challenge/run is command.

```bash
hacker@users~using-sudo:~$ sudo cat /flag
pwn.college{gT5TKmMkBAvSSgRvdHTAw-zdLQg.QX4UDN1wiNxIzNzEzW}
```
